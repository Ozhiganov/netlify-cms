...
import { entries, fill, isString, partition, pick, uniq, zipObject } from "lodash";
  ...
  updateTree(sha, path, fileTree) {
    return this.getTree(sha)
      .then(({ tree: dirContents }) => {
        ...
        const added = zipObject(updatedItems.map(item => item.path), fill([], true, 0, updatedItems.length));
